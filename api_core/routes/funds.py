...
def _fund_allocation_rows(code: str) -> list[dict]:
    ...
    def fetch():
        fund = Fund(code)
        info_alloc = fund.info.get("allocation")
        if isinstance(info_alloc, list):
            return info_alloc

        try:
            allocation_df = fund.allocation
            if allocation_df is not None and not allocation_df.empty:
                return df_to_json(allocation_df)
        except Exception:
            pass

        return []

    return get_cached_static(f"FUND_ALLOC_{code}", fetch)
...
def get_fund_estimated_return(request: Request, response: Response, code: str):
    ...
    def fetch():
        ...
        details = []
        mode = ""
        alloc = _fund_allocation_rows(code)
